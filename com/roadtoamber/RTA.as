...
import flash.filesystem.File;
    ...
    public class RTA extends UIComponent{
	...
	private const PORT:int = 6250;
	private const SERVER:String = "roadtoamber.com";
	...
	private var log:FileStream;

        public function RTA(characterName:String, passwordVal:String, output:TextArea) {
	...
	/**
	 * logs char in with specified name and password
   	 */
	public function login():void {
	    var connectStr:String = "connect "+character+" "+password+"\n";
	    msg("ConnectStr:"+connectStr);
	    var ba:ByteArray = new ByteArray();
            ba.writeMultiByte(connectStr, "UTF-8");
		...
		var hh:String = String(now.getHours());
		var mm:String = String(now.getMinutes());
		var timestamp:String = year + "." + month + "." + day +"-"+hh+mm;
		logfile = File.documentsDirectory;
		logfile = logfile.resolvePath(character+"-"+timestamp+".log");
		msg("logging to "+logfile.url);
		log = new FileStream();
		log.open(logfile, FileMode.WRITE);
